...
#### Django查找文件是惰性查找
+ STATICFILES_DIRS STATIC_ROOT STATIC_URL 差异
	+ STATICFILES_DIRS
		+ 如果没有设，Django会自动到各个app下的static文件夹里面查找静态文件
		+ 如果设置了，Django会先到STATICFILES_DIRS设定的目录查找静态文件， 其次再到各个app下的static文件夹里面查找
	+ STATIC_URL
		+ 利用前缀static_url来映射STATIC_ROOT,STATIC_URL用于引用STATIC_ROOT所指向的静态文件
	+STATIC_ROOT
		+ 部署静态文件时所有的静态文件聚合目录绝对地址
